...
class AVL_tree:
    ...
    def balance_LR(self, node):
        ...
        new_node_left_height = self.balance_factor(new_node['left'], new_node['left']['left']['height'] + 1 if (new_node['left']['left']) else 0, new_node['left']['right']['height'] + 1 if (new_node['left']['right']) else 0)
        new_node_right_height = self.balance_factor(new_node['right'], new_node['right']['left']['height'] + 1 if (new_node['right']['left']) else 0, new_node['right']['right']['height'] + 1 if (new_node['right']['right']) else 0)

        new_node['height'] = max(new_node_left_height + 1, new_node_right_height + 1)
            ...
            if (value < node['value']):
                ...
                if (not node['right']):
                    ...
                    if ((new_right_height - left_height) >= 2):
                        ...
                        node['height'] = max(new_right_height, left_height)
                        return node['height']
                        ...
                        if (delete_node['right']['left']):
                            ...
                            if (parent):
                                if (new_node['value'] < parent['value']):
                                    ...
                                    parent['right'] = new_node
                                new_node['parent'] = parent
                                return self.balance_factor(new_node, left_subTree['height'] + 1, new_node['right']['height'] + 1 if (new_node['right']) else 0)
                            else:
                                self.entrance = new_node
        ...
        def recursive(node, counter, result):
            ...
            if (node['right']):
                if (counter['c'] < len(self.nodes)/2):
                    result['a'] += self.data['1'][node['key']]
                else:
                    result['b'] += self.data['2'][node['key']]
                counter['c'] += 1

                if(node['middle']):
                    for key in node['middle']:
                        if (counter['c'] < len(self.nodes)/2):
                            result['a'] += self.data['1'][key]
                        else:
                            result['b'] += self.data['2'][key]
                        counter['c'] += 1
    ...
    def fix_height(self, node):
        def recursive(current, target):
            if (target['value'] < current['value']):
                new_left_height = recursive(current['left'], target) + 1
                right_height = current['right']['height'] + 1
                return self.balance_factor(current, new_left_height, right_height)
            elif (target['value'] > current['value']):
                new_right_height = recursive(current['right'], target) + 1
                left_height = current['left']['height'] + 1
                return self.balance_factor(current, left_height, new_right_height)
            elif (current == target):
                left_height = target['left']['height'] + 1 if(target['left']) else 0
                right_height = target['right']['height'] + 1 if(target['right']) else 0
                return self.balance_factor(target, left_height, right_height)
        recursive(self.entrance, node)
